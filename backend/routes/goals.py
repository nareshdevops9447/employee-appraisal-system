...
def push_templates_to_team():
    ...
    # Determine if this is a probation cycle for target_date logic
    is_probation = cycle.cycle_type == 'probation' if cycle else False

    created = 0
    updated = 0

    for employee in target_employees:
        # Calculate individual target date for probation (3 months from start)
        goal_target_date = end_date
        if is_probation and employee.start_date:
            goal_target_date = employee.start_date + relativedelta(months=3)
        ...
        for tmpl in templates:
            ...
            if existing:
                ...
                # Only update target date if it's currently generic
                if is_probation:
                    existing.target_date = goal_target_date
                updated += 1
                continue
